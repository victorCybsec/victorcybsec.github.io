...
In this walkthrough, I show how I obtained full access to Walla from OffSec Proving Grounds.

## Learning Objectives

- CVE-2020-24572
- Python Scripting + SUDO Abuse
...
Trying to navigate to `http://192.168.73.97:8091` prompts us with a Basic Authentication pop up. Tried basic credentials but did not work.

On our nmap scan we could see `Basic realm=RaspAP`. After a search for `RaspAP default credentials` we gained access to the service with `admin:secret`.

Once inside, we could make further enumeration about the service and navigating to `About RaspAP` page reveals the RaspAP version (`RaspAP v2.5`).

![alt text](img/8091.png)

## Exploitation

After a search for `raspap 2.5 exploit github` I found [this](https://github.com/lb0x/cve-2020-24572) exploit.

After reading the exploit and testing simpler commands, I made the following changes.
...
> Note: The second command is executed once we press `<ENTER>` (Already having ran the exploit).

Basically we host a reverse shell on our attacker machine, bring it to our victim using `wget`, make it executable and execute it.
...
The `shell` file contains the following.
...
The user **www-data** has **SUDO** permissions to execute `/usr/bin/python /home/walter/wifi_reset.py`.
...
Reading the file `/home/walter/wifi_reset.py` we can see it tries to import `wificontroller` and it does not exist in the program directory (which is the first directory where `python` tries to search for it). So we create our malicious library and give to it executable permissions (`chmod +x`).
...
This malicious library sets the SUID bit on the `/bin/bash` binary.
